...
pca.frame = data.frame(cbind(country=woMaltafiltered$Country, unclass(eu.pca$x)))
    ...
    plot(eu.pca[["rotation"]][,1], eu.pca[["rotation"]][,2], xlab="PC1",ylab="PC2", main="First Two PCAs' Transformation", pch=9); abline(v=0); abline(h=0) # new axes
    text(eu.pca[["rotation"]][,2]~eu.pca[["rotation"]][,1], labels=row.names(eu.pca[["rotation"]]), adj=c(-0.2, -0.2),cex=0.55)
...
The residual terms $\epsilon_i$ have mostly constant variance as the x-variables change (with a few exceptions) and are generally normally distributed according to the QQ-plot. The Shapiro-Wilk test for normality has a p-value that is not statistically significant at the level $\alpha = 0.05$, so we accept the null hypothesis: the errors are normally distributed. 
