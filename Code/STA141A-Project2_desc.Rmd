...
The final fitted model is as follows: $$Y_i = -0.0580 + 0.00094X_{obesity,i} + 0.00195X_{elderly,i} + 0.0000012X_{healthcare,i} + 0.00112X_{AQI,i} + 0.0000110X_{density,i} + \epsilon_{i}$$ where $i$ denotes the $i$'th country observed, and $X_{predictor}$ is the vector of observed values for a specific predictor. 
...
Higher proportions of elderly people in a population, as well as increases in the air quality index or the amount of money spent on healthcare per capita, are all associated with higher COVID-19 mortality rates at the national level. Elderly populations are at higher risk of dying from COVID-19, as are individuals who have suffered increased lung damage from more airborne particulates than normal. The increased mortality rate among countries with higher healthcare spending per capita may be due to the country's healthcare system being less efficient at allocating resources, or the healthcare system already operating near capacity even before the COVID-19 pandemic, and becoming overwhelmed as more patients and staff require new equipment and medical attention. 
...
Based on the fitted-to-residuals plot, the residual terms $\epsilon_i$ seem to have mostly constant variance as the x-variables change (with a few exceptions) and appear generally normally distributed according to the QQ-plot. The Shapiro-Wilk test for normality has a p-value that is not statistically significant at the level $\alpha = 0.05$, so it seems reasonable to accept the null hypothesis: the errors are normally distributed. 

The variance inflation factor has a value of over 5 for HCSpentPerCap, so there is some multicollinearity between the variables. After removing some similar variables (as determined by clusters in the PCA), the VIF is diminished enough to reasonably assume a lack of multicollinearity. 

```{r, echo=FALSE}
plot(mortnoMaltaEtc, which=1)
plot(mortnoMaltaEtc, which=2)
shapiro.test(resid(mortnoMaltaEtc))
vif(mortnoMaltaEtc)
