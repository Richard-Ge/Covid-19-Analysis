...
pop.data = read.csv("~/Documents/STA141A-pop-data.csv", row.names=1)
pop.data = pop.data[85, 1] = "Dominica"
sort(pop.data$Country)
newcovid = left_join(covid, pop.data, by = c("Country"))
# View(newcovid); View(pop.data)   # confused: saw the same pop. many times
sum(is.na(newcovid$Population))
...
recent.data = filter(covid21, Date_reported == "2021-02-20")
total.cases = sum(recent_data$Cumulative_cases)
total.deaths = sum(recent_data$Cumulative_deaths)
total.mortality = total_deaths/total_cases
## TODO: look at all the NA elements in recent_data, replace the names in pop.data with the new names in recent.data
View(recent.data); View(pop.data)
```
